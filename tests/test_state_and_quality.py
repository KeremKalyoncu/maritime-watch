...
def test_two_official_announcements_same_city_stay_separate(tmp_path):
    ...
    a.sources.append(
        Source(
            kind="official", org="SG", detail="İzmir önlerinde 2 şahıs", url="https://sg.gov.tr/izmir-2-sahis"
        )
    )
    s.upsert_incident(a)

    b = Incident(id="b", lat=38.43, lon=27.14, area="Güney Ege", places=["Izmir"], coarse=True, casualties=20)
    b.sources.append(
        Source(
            kind="official",
            org="SG",
            detail="İzmir açıklarında 20 göçmen",
...
def test_vessel_track_is_capped_to_what_the_rules_read(tmp_path):
    vs = VesselState(str(tmp_path / "v.json"), 5)
    for i in range(20):
        vs.update([{"mmsi": 1, "lat": 41.0, "lon": 29.0, "sog": 5, "ts": f"2026-09-05T10:{i:02d}:00Z"}])
...
def test_backfill_repairs_records_parsed_by_an_older_extractor(tmp_path):
    ...
    inc.sources.append(Source(kind="official", detail="03.09.2026 Muğla Açıklarında 2 Şahıs Kurtarılmıştır"))
...
def _wx(wid, area="Marmara Denizi", kind="marine-weather"):
    ...
    return Warning(
        id=wid, headline=f"{area}: fırtına", area=area, kind=kind, severity="major", org="Open-Meteo"
    )
...
def test_earthquakes_are_never_lifted_this_way(tmp_path):
    ...
    s = _store(tmp_path)
    s.upsert_warning(
        Warning(id="eq-1", headline="Deprem M4.2", area="Ege Denizi", kind="earthquake", org="AFAD")
    )
