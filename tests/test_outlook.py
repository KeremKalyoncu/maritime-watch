...
def test_outlook_message_includes_harbour_return_line(cfg):
    ...
    o = build("Marmara Denizi", _times(12), [8] * 6 + [26] * 6, [0.3] * 12, klass, hours=12, tz_offset_h=0)
    n = Notifier(cfg)
    sent = []
...
def test_outlook_off_by_config_sends_nothing(cfg, monkeypatch):
    ...
    monkeypatch.setattr(run, "fetch_forecast_points", lambda c: pytest.fail("must not fetch when disabled"))
    run.send_daily_outlook(cfg, None, dry=True)
