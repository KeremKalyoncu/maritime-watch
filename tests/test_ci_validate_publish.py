...
def test_outlook_ok_with_coverage(web_data):
    _write(web_data, "outlook.json", {
        "schema_version": 1,
        "generated": "2026-09-19T12:00:00Z",
        "coverage": {"expected": 2, "present": 1, "missing": ["X"]},
        "classes": {
            "small": {"areas": [{"name": "A", "windows": [{"start": "06:00", "end": "12:00",
                                                           "level": "ok", "gust_kn": 8, "wave_m": 0.3}]}]},
            "medium": {"areas": []},
            "large": {"areas": []},
        },
    })
...
def test_safety_rejects_unknown_perfect_score(web_data):
    _write(web_data, "safety_index.json", {
