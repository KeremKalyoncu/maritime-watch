...
def test_places_and_institutions_are_not_names():
    for t in (
        "Marmara Denizi'nde fırtına, Ege Denizi'nde ulaşıma ara verildi",
...
def test_aftermath_headlines_are_dropped(cfg):
    a = _after(cfg)
    for t in (
        "Silivri gemi kazasında kaptan tutuklandı",
...
def test_live_hazard_headlines_are_kept(cfg):
    a = _after(cfg)
    for t in (
        "Marmara'da tekne alabora oldu, 3 kişi kayıp",
...
def test_news_pipeline_publishes_no_person_name(cfg, monkeypatch):
    ...
    monkeypatch.setattr(
        _net.requests, "get", lambda *a, **k: (_ for _ in ()).throw(requests.RequestException("offline"))
    )
...
def test_mojibake_is_repaired():
    ...
    for good in (
        "SON DAKİKA HABERİ: Silivri'deki kazada gemi battı",
        "TUĞBERK İMAMOĞLU GEMİSİ",
        "Çeşme açıklarında fırtına",
    ):
...
def test_victim_context_does_not_eat_place_names():
    for t in (
        "Zonguldak açıklarında batan gemide kayıp 3 kişi aranıyor",
