...
def test_warning_same_id_refreshes_not_duplicates(tmp_path):
    s = _store(tmp_path)
    assert (
        s.upsert_warning(Warning(id="w1", headline="dalga ~2.7 m", org="Open-Meteo", value=2.7))[1] == "new"
    )
    cur, how = s.upsert_warning(Warning(id="w1", headline="dalga ~0.4 m", org="Open-Meteo", value=0.4))
...
def test_same_org_refetch_is_not_a_confirmation(tmp_path):
    ...
    s.upsert_warning(
        Warning(
            id="om-antalya",
            headline="dalga ~2.7 m",
            kind="marine-weather",
            org="Open-Meteo",
            area="Antalya Körfezi",
            value=2.7,
            lat=36.55,
            lon=30.6,
            issued="2026-09-04T01:00:00",
        )
    )
    cur, how = s.upsert_warning(
        Warning(
            id="om-antalya-later",
            headline="dalga ~0.2 m",
            kind="marine-weather",
            org="Open-Meteo",
            area="Antalya Körfezi",
            value=0.2,
...
def test_warning_merges_same_hazard_from_two_sources(tmp_path):
    ...
    a = Warning(
        id="eq-afad",
        headline="Deprem M4.3 - Marmara",
        kind="earthquake",
        org="AFAD",
        lat=40.91,
        lon=28.24,
        value=4.3,
        issued="2026-09-04T09:12:33",
    )
    b = Warning(
        id="eq-usgs",
        headline="Deprem M4.2 - Sea of Marmara",
        kind="earthquake",
        org="USGS",
        lat=40.91,
        lon=28.25,
        value=4.2,
        issued="2026-09-04T09:13:00",
    )
...
def test_distant_quakes_not_merged(tmp_path):
    ...
    s.upsert_warning(
        Warning(
            ...
            lon=28.2,
            value=4.3,
            issued="2026-09-04T09:12:00",
        )
    )
    _, how = s.upsert_warning(
        Warning(
            id="q2",
            headline="y",
            kind="earthquake",
